...
class ElearningLms extends Model {
	...
	* Author : Polo
	* Created Date : 25/03/11
	* Modified Date : 25/03/11
	* Version : 1.0
	* Function : findPageAll
	* Description : Retourne la liste des cours par page
	* 
	***********************************/
	public function findPageAll($conditions, $params)
	{
		$limit = 5;
		$page = 1;
		...
		$db = DbConn::getInstance();
		$query = $db->query(
			"SELECT c.idCourse, c.increment, c.course_type, c.idCategory, c.code, c.name, c.description, c.difficult, c.status AS course_status, c.course_edition, "
			."	c.max_num_subscribe, c.create_date, "
			."	c.direct_play, c.img_othermaterial, c.course_demo, c.use_logo_in_courselist, c.img_course, c.lang_code, "
			."	c.course_vote, "
			."	c.date_begin, c.date_end, c.valid_time, c.show_result, c.userStatusOp, c.auto_unsubscribe, c.unsubscribe_date_limit, "
			...
			." JOIN ".$GLOBALS['prefix_lms']."_courseuser AS cu ON (c.idCourse = cu.idCourse) "
			." WHERE ".$this->compileWhere($conditions, $params)
			.($_SESSION['id_common_label'] > 0 ? " AND c.idCourse IN (SELECT id_course FROM ".$GLOBALS['prefix_lms']."_label_course WHERE id_common_label = '".$_SESSION['id_common_label']."')" : "")
			." ORDER BY c.idCourse"
			." LIMIT $offset, $limit"
		);
		...
		if(!empty($courses))
		{
			...
			// find first LO type
			$re_firstlo = $db->query(
				"SELECT o.idOrg, o.idCourse, o.objectType FROM ".$GLOBALS['prefix_lms']."_organization AS o "
				." WHERE o.objectType != '' AND o.idCourse IN (".implode(',', $courses).") "
				." GROUP BY o.idCourse ORDER BY o.path"
			);
