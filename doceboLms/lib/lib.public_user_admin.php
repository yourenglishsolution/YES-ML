...
class PeopleListView extends ListView {
	...
	function _getSuspendImage() { return getPathImage('framework').'standard/suspend.gif'; }
	...
	function _getSortImage( $fieldname ) {
		if( isset($this->arr_fields_order[$fieldname] ) ) {
			if( $this->arr_fields_order[$fieldname] ) {
				// DESC
				return getPathImage('fw').'directory/1uparrow.png';
			} else {
				// ASC
				return getPathImage('fw').'directory/1downarrow.png';
			}
		} else {
			return getPathImage('fw').'directory/sort.png';
		}
	}
	...
	function _getRecoverImage() { return getPathImage('framework').'standard/recover.gif'; }
	...
	function _getRemoveImage() { return getPathImage('framework').'directory/de_assoc.gif'; }
	...
	function PeopleListView($title, &$data, &$rend, $id) {
		...
		$this->mod_perm = checkPerm('edituser_org_chart', true);
		$this->del_perm = checkPerm('deluser_org_chart', true);
	...
	function _getCols() {
		...
		for( $i = 0; $i < $this->nFields; $i++ ) {
			$colInfos[] = array( 	'hLabel' => Form::getInputDropdown(
												...
												$this->cFields[$i],
												'onchange="window.document.forms[0].submit()"'
												)
	...
	function fetchRecord() {
		...
		if( !$this->selector_mode ) { // Normal mode
			...
			if(($lev_current_user == ADMIN_GROUP_GODADMIN) || !isset($this->admins_user[$arrResult['idst']])) {
				...
				$arrResult['edit'] = '<a href="index.php?modname=public_user_admin&op=org_manageuser&id_user='.$arrResult['idst'].'&ap=mod_profile" '
			...
			if($arrResult['idst'] != $this->anonymous_idst) {
				...
				$arrResult['delete'] = '<input type="image" class="tree_view_image" '
							...
							.' id="'.$this->id.'_'.$this->_getOpDeleteItemId().'_'.$userid.'" '
							.' name="'.$this->id.'['.$this->_getOpDeleteItemId().']['.$userid.']" '
	...
	function userExtraData($id_user) {
		...
		$profile->init('profile', 'framework', 'modname=public_user_admin&op=org_manageuser&id_user='.$id_user, 'ap');
...
//		addScriptaculousJs();
		...
		$GLOBALS['page']->add('<script type="text/javascript">'
			.' setup_directory(\''.$GLOBALS['where_lms_relative'].'/modules/directory/ajax.public_user_admin.php\'); '
			.'</script>', 'page_head');
...
class GroupListView extends ListView {
	...
	function GroupListView($title, &$data, &$rend, $id) {
		...
		$this->mod_perm = checkPerm('editgroup', true);
		$this->del_perm = checkPerm('delgroup', true);
		$this->associate_perm = checkPerm('associate_group', true);
	}
...
class GroupMembersListView extends ListView {
	...
	function _getCols() {
		...
		$totCol = $this->data->getFieldCount();
		$colInfos = array();
		$colInfos[] = array( 	'hLabel' => '' /*<img src="'.$this->_getMemberTypeImage().'" '
										.'alt="'.$this->lang->def( '_DIRECTORY_MEMBERTYPE' ).'" '
										.'title="'.$this->lang->def( '_DIRECTORY_MEMBERTYPE' ).'" />'*/,
								'hClass' => "image",
								'fieldClass' => "image",
								'data' => 'itemtype',
								'toDisplay' => true,
