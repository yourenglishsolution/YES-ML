<?php
	...
	foreach($user_coursepath as $id_path => $path_info)
	{
		echo	'<div class="coursepath_container">'
				...
				.'</div>'
				.'</div>'
				.'<div id="courses_link_'.$id_path.'" class="coursepath_action"><a class="no_decoration" href="javascript:;" onclick="expandCourses(\''.$id_path.'\');"><span class="expand_path_info">'.Lang::t('_EXPAND', 'coursepath').'</span> '.Get::img('course/expand.png', Lang::t('_EXPAND', 'course')).'</a></div>';
		...
		foreach($coursepath_courses[$id_path] as $id_course => $course_info)
		{
			...
			echo	'<tr>'
					...
					.'<td class="course_type">'.Lang::t('_'.strtoupper($course_info['course_type']), 'coursepath').'</td>'
