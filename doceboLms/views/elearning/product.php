	...
	<div class="product">
		...
		<table class="command">
			...
			<tr class="line">
				...
				<td class="content"><?=ProductLms::priceFormat(ProductLms::getTTC($product->amount_ht))?> &euro;</td>
			</tr>
			<? if($product->discount_rate > 0) { ?>
			<tr class="line">
				<td class="label">Remise spéciale Monster (<?=($product->discount_rate*100)?>%)</td>
				<td class="content">- <?=ProductLms::priceFormat(ProductLms::doDiscount(ProductLms::getTTC($product->amount_ht), $product->discount_rate, true))?> &euro;</td>
			</tr>
			<? } ?>
			<tr class="total">
				<td class="label">Total à payer / mois (durant <?=$product->abo_months?> mois)</td>
				<td class="content"><?=ProductLms::priceFormat(ProductLms::doDiscount(ProductLms::getTTC($product->amount_ht), $product->discount_rate))?> &euro;</td>
			</tr>
			<tr>
				...
				<td class="none payment">
					...
					<form id="paypalForm" action="<?=_PAYPAL_ACCOUNT_URL?>" method="post">
						...
						<input type="hidden" name="a3" value="<?=ProductLms::doDiscount(ProductLms::getTTC($product->amount_ht), $product->discount_rate)?>">
