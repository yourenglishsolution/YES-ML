...
YAHOO.util.Event.addListener(window, "load", function() {
	...
	docebo_quest_bank = new function() {
		...
		var myColumnDefs = [
			...
			{key:"del_quest", 		label:QB_DEF.del_quest_img, 	formatter:this.formatDelQuest }
		];
		if(use_mod_action) {
		...
		var myPaginator = new YAHOO.widget.Paginator({
			...
			pageLinks 				: 5,
			rowsPerPage 			: quest_per_page,
			rowsPerPageOptions 		: [parseInt(quest_per_page/2),quest_per_page,quest_per_page*2],
			alwaysVisible 			: false,
		...
		var oConfigs = {
			initialRequest         : 'startIndex=0&results=' + quest_per_page,
