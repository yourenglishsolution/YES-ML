...
ScormApi.prototype.commitAnswer = function( data ) {
	
	var postdata = 'aicc_data='+data+'&idscorm_item='+this.idscorm_item+'&idUser='+this.idUser;
	var ajxreq = new Ajax.Request(
       	this.baseurl+'?op=CommitAnswer',
       	{	method: 'post',
			asynchronous:false,
			postBody: postdata,
			requestHeaders: {
				'Man':"POST " + this.baseurl + " HTTP/1.0",
				'Host':this.host,
