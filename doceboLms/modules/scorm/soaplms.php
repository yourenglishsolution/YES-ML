...
class SOAPLMS {
	...
	function Finish( $idUser, $idReference, $idscorm_item, $environment = 'course_lo' ) {
		...
		* Author : Polo
		* Created Date : 21/03/11
		* Modified Date : 21/03/11
		* Version : 1.0
		* Function : Requete SQL
		* Description : Changement du flag dans la table de lien entre les utilisateurs et les cours
		* 
		**********************************/
		if($idUser > 0 && $idscorm_item > 0)
		{
			$db =& DbConn::getInstance();
			$sql = "SELECT idCourse FROM ".$GLOBALS['prefix_lms']."_organization WHERE idResource=".$idscorm_item;
			$row = $db->fetch_row($db->query($sql));
			
			$sql = "UPDATE ".$GLOBALS['prefix_lms']."_courseuser SET status=2 WHERE idUser=".((int)$idUser)." AND idCourse=".((int)$row[0]);
			$db->query($sql);
...
if( (isset($_GET['op']) && $_GET['op'] == 'Finish') ) {
	...
	$xmlRequest->loadXML($GLOBALS['HTTP_RAW_POST_DATA']);
 ...
 */	
	 ...
	 * Created Date: 18/04/2011
	 * Modified Date: 18/04/2011
	 * Function: scoload
	 * Description: load idUser and idscorm_item in parameters
	 ************************************/
	$parameters = "?idUser=".$_GET['idUser']."&idscorm_item=".$_GET['idscorm_item'];
...
} else if( (isset($_GET['op']) && $_GET['op'] == 'CommitAnswer') ) {
	if (!empty($_POST['aicc_data']) && !empty($_POST['idUser']) && !empty($_POST['idscorm_item'])) {
		$query = "INSERT INTO yes_learning_user_item_answer"
				." (idUser, idscorm_item, aicc_data, created_date) VALUES"
				." ('".$_POST['idUser']
				."','".$_POST['idscorm_item']
				."','".$_POST['aicc_data']
				."', now())";
