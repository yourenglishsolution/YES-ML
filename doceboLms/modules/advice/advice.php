...
function adviceList() {
	...
	if(!empty($advice_all)) {
		...
		if(mysql_num_rows($re_advice) >= 1 && $mod_perm) {
...
function advice() {
	...
	* Author : Polo
	* Created Date : 31/03/11
	* Modified Date : 31/03/11
	* Version : 1.0
	* Function : Affichage
	...
	**********************************/
	...
	cout('<div class="errorMsg">Une erreur est survenue. <a href="index.php?r=elearning/show&sop=unregistercourse">Cliquez ici</a> pour revenir sur l\'accueil</div>');
...
function updreader() {
	...
	createNewAlert(	'AdviceNew', 
					...
					'add', 
					'1', 
					'Inserted advice '.$_POST['title'].' in course '.$_SESSION['idCourse'], 
					$dest, 
					$msg_composer );
	}
...
function archiveadvice() {
	...
	if(mysql_num_rows($re_my_advice)) {
