 ...
 * Author : Polo
 * Created Date : 01/04/11
 * Modified Date : 01/04/11
 * Version : 1.0
 * Function : lostpwd
 * Description : Modification de l'affichage de la fonction
 * 
 ***********************************/
function lostpwd() {
	...
	$GLOBALS['page']->add('<h2>'.strtoupper($lang->def('_LOGIN')).'</h2>', 'content');
	
	if($user_manager->haveToLostpwdConfirm())	$GLOBALS['page']->add($user_manager->performLostpwdConfirm(), 'content');
	if($user_manager->haveToLostpwdAction())	$GLOBALS['page']->add($user_manager->performLostpwdAction('index.php?modname=login&amp;op=lostpwd'), 'content');
	if($user_manager->haveToLostpwdMask())		$GLOBALS['page']->add($user_manager->getLostpwdMask('index.php?modname=login&amp;op=lostpwd'), 'content');
}
