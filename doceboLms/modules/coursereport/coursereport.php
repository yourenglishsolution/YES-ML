				...
				$query_report = "
						...
						while($report = sql_fetch_assoc($result))
						{
							if($report['score_raw']!=NULL)
							{
								...
								$total=$total+1;
								if($report['entry']=='completed' or $report['entry']=='passed' )
									$passed=$passed+1;
							}
						}
	...
	if(checkPerm('mod', true)) {
		$out->add(
			...
			'<ul class="link_list_inline">'
				...
				.'<span>'.$lang->def('_EXPORT_XLS').'</span></a></li>
			</ul>'
			.'</div>'
		);
	}
...
function showchart() {
	...
	$idTest = get_req('id_test', DOTY_INT, -1);
	$idUser = get_req('id_user', DOTY_INT, -1);
	$chartType = get_req('chart_type', DOTY_STRING, 'column');
