...
function delforum() {
	...
	if(isset($_GET['confirm'])) {
		...
		while(list($idT) = sql_fetch_row($re_thread)) {
			...
			WHERE idThread = '$idT'")) Util::jump_to('index.php?modname=forum&op=forum&result=err_del');
			unsetNotify('thread', $idT);
		}
		if(!sql_query("
		DELETE FROM ".$GLOBALS['prefix_lms']."_forumthread
		WHERE idForum= '".$id_forum."'")) Util::jump_to('index.php?modname=forum&op=forum&result=err_del');
		if(!sql_query("
...
function addthread() {
	...
	$GLOBALS['page']->add(
		...
		.Form::openForm('form_forum', 'index.php?modname=forum&amp;op=insthread', false, false, 'multipart/form-data')
		.Form::openElementSpace()
