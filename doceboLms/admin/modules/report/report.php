...
function load_filter($id, $tempdata=false, $update=false) {
	...
	$row = sql_fetch_assoc(sql_query("SELECT * FROM ".$GLOBALS['prefix_lms']."_report_filter WHERE id_filter=$id"));
...
function get_report_table($url='') {
	...
	//filter by author
	YuiLib::load();
	$filter = Get::req('author_filther', DOTY_INT, 0);
	...
	switch ($level) {
		case ADMIN_GROUP_GODADMIN : {
			...
			if($filter > 0) $query .= " WHERE t1.author = ".$filter;
		};break;
		...
		default : {
			 ...
			 if($filter > 0) $query .= " WHERE ( t1.author = ".$filter." AND t1.is_public = 1 ) ";
			 else $query .= " WHERE ( t1.author = ".getLogUserId()." OR t1.is_public = 1 ) ";
		}break;
	}
	...
	if ($res = sql_query($query)) {
		while ($row = sql_fetch_assoc($res)) {
			...
			$export_link_xls = '<a class="ico-sprite subs_xls" href="'.$export_url.'&dl=csv" title="'.Lang::t('_EXPORT_XLS', 'report').'"><span></span>'.Lang::t('_EXPORT_XLS', 'report').'</a>';
...
function setup_report_js() {
	...
	Util::get_js($GLOBALS['where_lms_relative'].'/admin/modules/report/ajax.report.js', true, true);
}
...
function report_show_results($idrep = false) {
	...
	if($idrep == false) {
		...
		$res = sql_query("SELECT class_name, file_name FROM ".$GLOBALS['prefix_lms']."_report WHERE id_report=".$id_report." AND enabled=1");
		$author = 0;
