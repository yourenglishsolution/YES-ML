...
function courses_create_filter(e) {
  ...
  switch (type) {
    case filter_cases._FILTER_DATE: {
      ...
      try { dat.type='button'; } catch(e) {}
      dat.className='trigger_calendar';
      dat.id='trigger_'+index;
      ...
      t.appendChild(dat);
      t.appendChild(inc);
      cals.push(new Calendar.setup({
				inputField : val.id, // ID of the input field
				ifFormat : course_date_token, // the date format
				button : dat.id, // ID of the button
				showsTime : false }) );
    } break;
