...
class Report_User extends Report {
	...
	function Report_User($id_report, $report_name = false) {
		...
		$this->LO_columns=array(
			array('key'=>'userid',          'select'=>false, 'group'=>'user',   'label'=>$this->lang->def('_USERNAME')),
	...
	function get_courses_filter() {
		...
		$out->add('<script type="text/javascript">'."\n".
			//'_temp_seldata='.$js_seldata.';'."\n".
			  ...
			  'var course_date_token=\''.$regset->date_token.'\';'."\n".
	...
	function _get_courses_query($type='html', $report_data = NULL, $other='') {
		...
		if( $userlevelid != ADMIN_GROUP_GODADMIN ) {
			...
			if (isset($admin_courses['course'][0])) { //the sub admin can see all courses - no filter required
				//...
			} else {
				if ($filter_allcourses) {
					$filter_allcourses = false;
					$rs = sql_query("SELECT idCourse FROM %lms_course");
					$filter_courseselection = array();
					while (list($id_course) = sql_fetch_row($rs)) { $filter_courseselection[] = $id_course; }
				}
				if (!empty($admin_courses['course'])) {
					$_clist = array_values($admin_courses['course']);
					$filter_courseselection = array_intersect($filter_courseselection, $_clist);
				} else {
					$filter_courseselection = array();
				}
			}
		...
		if($org_chart_subdivision == 0) {
			...
			if ($show_classrooms_editions) {
				$query_course_user = "SELECT cu.idUser, c.code, c.idCourse, c.idCategory, c.name, c.status AS course_status, cu.level, "
					." cu.status, cu.date_inscr, cu.date_first_access, cu.date_complete, c.mediumTime, c.course_type, "
				...
				$query_course_user = "
				SELECT cu.idUser,
					c.code, c.idCourse, c.idCategory, c.name, c.status AS course_status, cu.level,
					cu.status, cu.date_inscr, cu.date_first_access, cu.date_complete, c.mediumTime,
					c.course_type
				FROM  %lms_courseuser AS cu
					JOIN %lms_course AS c ".   ", %adm_user as u "."
			...
			while(list($idst_group, $folder_name) = each($org_name)) {
				...
				$GLOBALS['page']->add( '<b>'.$lang->def('_CREATION_DATE').' :</b> '.$date_now.'<br />'
					.'</div>', 'content');
		...
		while($sql_row =  sql_fetch_array($re_course_user) ) {
			...
			if ($show_classrooms_editions) {
				list($id_user, $code, $id_course, $id_category, $name, $status, $level_user,
					$status_user, $date_inscr, $date_first_access, $date_complete, $medium_time, $course_type,
					$id_date, $classroom_date_subscription, $classroom_date_complete) = $sql_row;
			} else {
				list($id_user, $code, $id_course, $id_category, $name, $status, $level_user,
					$status_user, $date_inscr, $date_first_access, $date_complete, $medium_time, $course_type) = $sql_row;
			}
			...
			if ($condition) {
				...
				$row = array();
				$row[] =  $acl_man->relativeId($users_info[$id_user][ACL_INFO_USERID]);
				if (in_array('_TH_LASTNAME', $cols)) $row[] =  $users_info[$id_user][ACL_INFO_LASTNAME];
				if (in_array('_TH_FIRSTNAME', $cols)) $row[] =  $users_info[$id_user][ACL_INFO_FIRSTNAME];
				if (in_array('_TH_EMAIL', $cols)) $row[] = $users_info[$id_user][ACL_INFO_EMAIL];
				if (in_array('_TH_REGISTER_DATE', $cols)) $row[] = Format::date($users_info[$id_user][ACL_INFO_REGISTER_DATE], 'datetime');
				...
				$row[] = $name;
				$row[] = $course_types_trans[$course_type];
...
//die('<pre>'.print_r($rc_filters, true).'</pre>');
	...
	function get_delay_filter() {
		...
		.Form::getDatefield($dlang->def('_DATE_UNTIL_COURSE_END'), 'date_until_course_end', 'date_until_course_end', Format::date($ref['date_until_course_end'], 'date') )
		//.Form::getCloseFieldset()
	...
	function _get_delay_query($type='html', $report_data = NULL, $other='') {
		...
		if (1==1)//($org_chart_subdivision === 0)
		{
			...
			while(list($id_u, $id_c, $id_e, $date_inscr, $fisrt_access, $date_complete, $status, $level) = sql_fetch_row($re_course_user)) {
			...
			$this->_printTable_delay($type, $element_to_print, $cdata['showed_columns']);
			if ($this->use_mail) { $this->_loadEmailActions(); }
		}
	}
	...
	function _printTable_delay($type, &$element_to_print, $showed_cols=array()) {
		...
		{
			cout($lang->def('_NO_USER_FOUND'), 'content');
		}
		else
		{
			require_once(_lms_.'/admin/modules/report/report_tableprinter.php');
			...
			foreach ($element_to_print as  $id_course=> $info)
			{
				...
				$header = array(
					...
					$lang->def('_LASTNAME'),
					$lang->def('_FIRSTNAME'),
					array('style'=>'align-center', 'value'=>$lang->def('_STATUS')),
					$lang->def('_EMAIL'),
					array('style'=>'align-center', 'value'=>$lang->def('_USER_STATUS_SUBS')),
					array('style'=>'align-center', 'value'=>$lang->def('_DATE_FIRST_ACCESS')),
					array('style'=>'align-center', 'value'=>$lang->def('_DATE_COURSE_COMPLETED'))
				);
				...
				foreach($info['data'] as $user_info)
				{
						/*$output .=
							'<tr class="row'.( (++$i)%2 ? '' : '_col' ).'">'."\n\t"
								...
								.'<td class="align_center">'.Format::date($user_info['date_inscr'], 'datetime').'</td>'."\n\t"
								.'<td class="align_center">'.Format::date($user_info['first_access'], 'datetime').'</td>'."\n\t"
								.'<td class="align_center">'.Format::date($user_info['date_completed'], 'datetime').'</td>'."\n\t"
					...
					$line = array(
						...
						$user_info['lastname'],
						$user_info['firstname'],
						array('style'=>'align-center', 'value'=>$this->status_u[$user_info['status']]),
						$user_info['mail'],
						array('style'=>'align-center', 'value'=>Format::date($user_info['date_inscr'], 'datetime')),
						array('style'=>'align-center', 'value'=>Format::date($user_info['first_access'], 'datetime')),
						array('style'=>'align-center', 'value'=>Format::date($user_info['date_completed'], 'datetime'))
					);
					if ($this->use_mail) {
						$line[] = array(
							'style'=>'img-cell', 
	...
	function get_LO_filter() {
		...
		$res=sql_query("SELECT * FROM ".$GLOBALS['prefix_lms']."_lo_types");
	...
	function _get_LO_query($type='html', $report_data = NULL, $other='') {
		...
		if ($this->use_mail)
			$head1[] = array(
				'style' => 'img-cell',
				'value' => $this->_loadEmailIcon()
			);//'<img src="'.getPathImage().'standard/email.gif"/>';//'';
		...
		$res = sql_query($query); //cout('<div>'.$query.'</div>');cout('<div>'.mysql_error().'</div>');
		while ($row=mysql_fetch_assoc($res)) {
		...
		$LO_types = array();
		$res = sql_query("SELECT objectType FROM ".$GLOBALS['prefix_lms']."_lo_types");
		while (list($idtype) = sql_fetch_row($res)) $LO_types[$idtype] = $tlang->def('_LONAME_'.$idtype);
			...
			switch ($val['key']) {
				...
				case '_CUSTOM_FIELDS_': {
					foreach ($customcols as $field) {
						if ($field['selected']) {
							if ( isset($field_values[$field['id']][$row['userId']]) )
							$temp[] = $field_values[ $field['id'] ][ $row['userId'] ];
							else
							$temp[] = '';
						}
					}
