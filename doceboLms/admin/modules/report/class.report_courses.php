		...
		switch ($substep) {
			...
			case _SUBSTEP_COLUMNS: {
				...
				.Form::getDatefield($lang->def('_START_TIME'), 'start_time_'.$this->id_report, 'start_time',
					Format::date($ref['time_belt']['start_date'], 'date') )
				.Form::getDatefield($lang->def('_TO'), 'end_time_'.$this->id_report, 'end_time',
					Format::date($ref['time_belt']['end_date'], 'date') )
		...
		if (!$show_classrooms_editions) {
			...
			if($org_chart_subdivision == 0) {
				...
				$query_course_user = "SELECT cu.idUser, cu.idCourse, cu.date_first_access, cu.date_complete, cu.status, c.course_type, d.id_date "
					." FROM (%lms_courseuser AS cu JOIN %lms_course AS c ON (cu.idCourse = c.idCourse)) "
