...
class SubscriptionAlmsController extends AlmsController {
	...
	public function init() {
		...
		$this->permissions = array(
			...
			'moderate' => checkPerm('moderate', true, 'course')
		);
	...
	protected function _getMessage($code) {
		...
		switch ($code) {
			case "no permission": $message = "";
				break;
			case "no options selected": $message = "";
				break;
		}
		return $message;
	}
					...
					if ($can_subscribe) {
						...
						if(!empty($user_selected) && $send_alert) {
							...
							$array_subst = array(	'[url]' => Get::site_url(),
													...
													'[medium_time]' => Format::date(date("Y-m-d", time() + ($course_info['mediumTime']*24*60*60) ), 'date'));
	...
	public function fastsubscribe() {
		...
		if ($id_user <= 0) {
			$id_user = false;
			if ($userid != '')
				$id_user = $this->acl_man->getUserST($userid);
		}
 ...
 $output['message'] = $message;
	...
	public function cataloguesubscribeusersTask() {
		...
		if (isset($_POST['cancelselector'])) {
			...
			$user_selected = & $acl_man->getAllUsersFromIdst($_entity_selected);
			...
			if (count($editions_list) > 0) {
				...
				foreach ($user_selected as $user) {
					foreach ($courses_list as $idCourse => $course) {
						$data[] = array($user, $idCourse, false);
					}
				}
	...
	public function cataloguesubscribesaveTask() {
		...
		foreach ($user_selection as $user) {
			foreach ($courses_list as $idCourse => $course) {
				$data[] = array($user, $idCourse, array_key_exists($idCourse, $editions) ? $editions[$idCourse] : false);
			}
		}
	...
	protected function _subscribeUsersToCatalogue($data) {
		...
		foreach ($data as $value) {
			list($id_user, $id_course, $edition_id) = $value;
			$arr_courses[] = $id_course;
		}

		//die('<pre>'.print_r($arr_courses, true).'</pre>');

		$arr_editions = $edition_man->getCourseEditions($arr_courses);

		//die('<pre>'.print_r($arr_editions, true).'</pre>');
		...
		//die('<pre>'.print_r($arr_editions."ZUPPO<br>".$already_editions, true).'</pre>');


		//subscribe users
		$count_u = 0;
		$count_e = 0;
		$lv_user = 3; //only students can be managed for multiple inscriptions
		$waiting = false; //no waiting condition

		//die('<pre>'.print_r($data, true).'</pre>');

		foreach ($data as $value) {
			list($id_user, $id_course, $id_edition) = $value;
			...
			if ($id_edition) {
				if (!isset($already_editions[$id_edition][$id_user])) {
					...
					//die('<pre>'.print_r("Usertoed: ".$id_user.$id_course.$id_edition.$lv_user.$waiting, true).'</pre>');
					if ($res)
						$count_e++;
				}
			} else {
		...
		//die('<pre>'.print_r(" Users:".$count_u." editions:".$count_e, true).'</pre>');
		/*
		  //send alerts
	...
	public function show_coursepathTask() {
		...
		$message = false;
		switch ($res) {
			case 'ok': $message = UIFeedback::info(Lang::t(strtoupper($_GET['res']), 'subscription'));
				break;
			case 'err': $message = UIFeedback::error(Lang::t(strtoupper($_GET['err']), 'subscription'));
				break;
		}
	...
	public function multidel_coursepath() {
		...
		if ($users == '') {
			...
			$res = $this->model->unsubscribeFromCoursepath($id_path, $arr_user);
	...
	public function del_coursepathTask() {
		$id_path = get_req('id_path', DOTY_INT, 0);
		$id_user = get_req('id_user', DOTY_INT, 0);
