...
class UsermanagementAdmController extends AdmController {
	...
	public function init() {
		...
		$this->permissions = array(
			...
			'associate_user'		=> checkPerm('mod_org', true, 'usermanagement')
		);
		...
		if (is_array($list) && count($list)>0) {
			...
			foreach ($list as $idst=>$record) {
				...
				foreach ($var_fields as $i=>$value) {
					...
					//check if field type requires highlighting for $filter_text ... TO DO
					$record_row['_dyn_field_'.$i] = (isset($record[$name]) ? $record[$name] : '');
				}
	...
	protected function _getUserEditMask($idst = false) {
		...
		$fields_mask = $fman->playFieldsForUser($is_editing ? $idst : -1, false, false, true, false, false, !$is_editing ? Docebo::user()->getIdst() : false);
	...
	function createuser_action() {
		...
		if (!$this->_canUseLevel($userdata->level)) {
			$output['success'] = false;
			$output['message'] = Lang::t('_OPERATION_FAILURE', 'admin_directory');
			echo $this->echoResult($output);
			return;
		}
		...
		if (is_numeric($idst) && $idst>0) {
			...
			$output['message'] = Lang::t('_OPERATION_SUCCESSFUL', 'register').': '.$userid;
	...
	function suspend() {
		...
		if ($idst>0 && ($action==0 || $action==1)) {
			if ($action==0) {
				...
				$output['message'] = UIFeedback::pinfo(Lang::t('_OPERATION_SUCCESSFUL', 'admin_directory'));
			}
			else {
	...
	public function changepwd_actionTask() {
		...
		if ($idst <= 0) {
			...
			if ($_userid != $userid) {
				...
				if (!$idst) {
					$output['success'] = false;
					$output['message'] = UIFeedback::perror(Lang::t('_OPERATION_FAILURE', 'admin_directory'));
					echo $this->json->encode($output);
					return;
				}
			}
		...
		if ($res) {
			...
			$output['success'] = false;
			$output['message'] = UIFeedback::perror(Lang::t('_OPERATION_FAILURE', 'admin_directory').': <b>'.$userid.'</b>');
		}
		echo $this->json->encode($output);
	}
	...
	public function importusers() {
		...
		switch ($step) {
			...
			case 2: {
				...
				$dst = new ImportUser(array('dbconn'=>$GLOBALS['dbConn'], 'tree' => $idOrg));
	...
	public function csvexport() {
		...
		if ($users != "") {
			$acl_man = Docebo::user()->getAclManager();
			$arr_users = explode(',', $users);
			$details = $this->model->getUsersDetails($arr_users, true);
			if (is_array($details)) {
