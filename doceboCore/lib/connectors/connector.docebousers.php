	...
	**/
	...
	function add_row( $row, $pk ) {
		...
		if( $arr_user === FALSE ) {
			...
			$idst = Docebo::aclm()->registerUser( $userid, $firstname, $lastname,
																$pass, $email, '',
																'');
		} else {
			$idst = $arr_user['idst'];
			...
			$result = Docebo::aclm()->updateUser( $idst, $userid, $firstname, $lastname,
																$pass, $email, FALSE,
																FALSE, FALSE, TRUE);
			if( !$result ) {
		...
		if($idst !== false) {
			...
			//-----------------------------------------------------------
			if( !$result ) {
				$this->last_error = 'error in store custom fields';
			}
			return $result;
		} else {
			$this->last_error = 'error on register user';
			return FALSE;
		}		
	}
