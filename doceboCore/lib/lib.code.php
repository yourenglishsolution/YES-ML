...
class CodeManager
{
	...
	public function generateCode($id_code_group, $length = 10)
	{
		$step = 2;
			
		$all_code = $this->getAllCode();
		...
		while($control)
		{
			for($a = 0; $a < $length; $a++)
			{
				...
				if($seed > 10) $new_code .= mt_rand(0, 9);
				elseif($seed > 5) $new_code .= chr(mt_rand(65, 90));
				else $new_code .= chr(mt_rand(97, 122));
			}

			if(array_search($new_code, $all_code) === false)
			{
				$this->addCode($new_code, $id_code_group, 0);
				$control = false;
			}
		}
