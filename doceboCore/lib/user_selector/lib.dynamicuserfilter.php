...
class DynamicUserFilter {
	...
	private function getFieldsList($js = false) {
		...
		$fields = array_merge($this->getStandardFieldsList(), $this->getCustomFieldsList());
		...
		foreach ($fields as $val) {
			$temp[] = '{id: "'.$val['id'].'", name: "'.$val['name'].'", type: "'.$val['type'].'", standard: '.($val['standard'] ? 'true' : 'false').'}';    
		}
		return '['.implode(',', $temp).']';    
	}
	...
	public function get($domready = true, $tags = true) {
		...
		$js_function = 'YAHOO.namespace("dynFilter");'
			...
			.'		lang: {'
			.'      _ADD_FILTER: "'.Lang::t('_ADD', 'standard').'",'
			.'      _ADD: "'.Lang::t('_ADD', 'standard').'",'
			.'      _DEL: "'.Lang::t('_DEL').'",'
			.'      _RESET: "'.Lang::t('_RESET').'",'
			.'      _FILTER_ONE_COND: "'.Lang::t('_FILTER_ONE_COND').'",'
			.'      _FILTER_ALL_CONDS: "'.Lang::t('_FILTER_ALL_CONDS').'"'
			.'     }'."\n"
			.'	}); '."\n";
		...
		if ($this->_use_other_fields) {
			...
			.'YAHOO.otherFields = new DynamicFilterOtherFieldTypes({languages: '.$initotherfields['languages'].',courses: '.$initotherfields['courses'].',levels: '.$initotherfields['levels'].'});'
