...
function _oGroupDataTable(tableId, containerId, oConfig, oLangs) {
  this._oLangs = oLangs;
  this.init(tableId, containerId, oConfig);
}


_oGroupDataTable.prototype = {
  ...
  init: function(tableId, containerId, oConfig) {
    ...
    var formatCheckbox_Sel = function(elCell, oRecord, oColumn, oData) {
      //should purge previous checkboxes events
			elCell.innerHTML = '';
    ...
    var oColumnDefs = [
			...
			{key:"groupid",       label:this._oLangs. def('_NAME'),        sortable: true},
			{key:"description",  label:this._oLangs. def('_DESCRIPTION'), sortable: true}
		];
		...
		this._oDataSource.responseSchema = {
			...
			fields: ["idst", "groupid", "description"]//{key:"name", parser:YAHOO.util.DataSource.parseString}]
		};
		...
		this._oPaginator = new YAHOO.widget.Paginator({
			...
			rowsPerPage 	: oConfig.groups_per_page,
			alwaysVisible : true,
    ...
    var oInitialRequestParams = {
      startIndex: 0,
      results: oConfig.groups_per_page,
      sort: 'groupid',
      dir: 'asc'
    };
		...
		var oTableConfig = {
			...
			sortedBy               : {key: "groupid", dir: YAHOO.widget.DataTable.CLASS_ASC}, // Set up initial column headers UI
  ...
  _getRequestString: function(oParams) {
    var req = "op=table_get_groups&startIndex="+oParams.startIndex+"&results="+oParams.results+"&sort="+oParams.sort+"&dir="+oParams.dir;
...
function GroupSelectorTable(container, oConfig) {
...
GroupSelectorTable.prototype = {
  ...
  init: function(container, oConfig) {
    ...
    this._oTable = new _oGroupDataTable("table_container_"+this.id, this.id, oConfig, this._oLangs);
    ...
    if (this._serverUrl != "") {
      ...
      var oCallback = {
        success: function(o) {
          ...
          var nodes = YAHOO.util.Selector.query('input[id^=group_]');
  ...
  selectPage: function(e) { 
    var i, nodes = YAHOO.util.Selector.query('input[id^=group_]');
