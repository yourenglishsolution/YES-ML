		...
		if(!empty($teacher_course)) {
			...
			while(list($id, $data) = each($teacher_course)) {
				if ($this->userCourseSubscrived($id))
					$html .= '<li><a href="'.$GLOBALS['where_lms_relative'].'/index.php?modname=course&amp;op=aula&amp;idCourse='.$id.'">['.$data['code'].'] '.$data['name'].'</a></li>';
				else
...
class UserProfileData {
	...
	function getUserField($id_user) {
		...
		foreach($field_founded as $field_id => $value) {
			...
			if(isset($fal[$field_id])) {
				...
				//do not return the field if it has no policy assigned (default policy = not visible)
				/*
				$field[$field_id] = array(	'name' 	=> $value[0],
											'value' => $value[1] );
				*/
	...
	function saveUserData($id_user, $data, $also_preference, $also_extra_field) {
		...
		if(!$this->acl_man->updateUser( 	$id_user,
											...
											(isset($data['force_change']) ? $data['force_change'] : false),
	...
	function getUserCourseStat($id_user) {
		...
		while(list($id_c, $code, $name, $status,
			...
			switch($status_user) {
				case _CUS_WAITING_LIST : $stats[$id_c]['user_status'] = $lang->def('_WAITING_USERS'); break;
				case _CUS_SUBSCRIBED 	: $stats[$id_c]['user_status'] 	= $lang->def('_USER_STATUS_SUBS');break;
				case _CUS_BEGIN 		: $stats[$id_c]['user_status'] 	= $lang->def('_USER_STATUS_BEGIN');break;
				case _CUS_END 			: $stats[$id_c]['user_status'] 	= $lang->def('_USER_STATUS_END');break;
				case _CUS_SUSPEND 		: $stats[$id_c]['user_status'] 	= $lang->def('_USER_STATUS_SUSPEND');break;
				default : $stats[$id_c]['user_status'] = ""; break;
			}
