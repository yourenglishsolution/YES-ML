...
class Boot {
	...
	private static function loadSetting() {
		...
		if(defined("CMS")) Util::load_setting(Get::cfg('prefix_cms').'_setting', 'cms');
		if(defined("SCS")) Util::load_setting(Get::cfg('prefix_scs').'_setting', 'scs');
		*/
	}
	...
	private static function sessionCookie() {
		...
		ini_set('session.gc_maxlifetime', (isset($cfg['session_lenght'])) ? (int)$cfg['session_lenght'] : 200000 );
