...
if ($GLOBALS['ClassToInstanciate'] == 'QuestionManager')
{
	class QuestionManager
	{
		...
		function QuestionManager()
		{
			...
			$this->Answer[0] = "savings";
			
			$this->Answer[1] = "re-emerge";
		...
		function GetScore()
		{
			...
			for ($n = 0; $n < count($this->Answer); $n++) 		
			{
				...
				if ($this->Answer[$n] == stripslashes($_GET["M".($n+1)]))
					$score++;
				else
					$score--;
			}
	...
	class QuestionDisplayer extends QuestionDisplayerBase
	{
		...
		function PrintCorrection($bInCorrection)
		{
			if ($bInCorrection)
			{
				...
				if ($Score != $ScoreMax)
				{
					...
					$AnswerText .= "to have a nest egg<br> &rarr; savings<br>to stage a comeback &rarr; re-emerge<br>to squirrel away &rarr; hoard<br>";
				}
		...
		{	
			...
			if (isset($Answers[$SelectName]))
			{
				...
				return (stripslashes($Answers[$SelectName]) == $OptionText) ? "selected" : "";
			}
...
<style type="text/css">
	...
	</style><script type="text/javascript" language="javascript" src="_scripts/ResetAllCombos.js"></script><meta http-equiv="imagetoolbar" content="no">
...
<td class="Question_text ProcessInlineImages" valign="top">Match the expressions on the left with the words on the right.</td>
</tr>
<tr class="Question_text">
...
<tr class="AnswerRow1">
<td class="Question_answers ProcessInlineImages">to have a nest egg<br> </td>
<td class="Question_answers" valign="middle"><select onclick="resetAllCombos(this);" size="1" class="TAT_Selects" name="M1"><option value="" <?=$this->IsSelected($bInCorrection, "M1", "")?> ></option><option value="hoard" <?=$this->IsSelected($bInCorrection, "M1", "hoard")?> >hoard</option><option value="re-emerge" <?=$this->IsSelected($bInCorrection, "M1", "re-emerge")?> >re-emerge</option><option value="savings" <?=$this->IsSelected($bInCorrection, "M1", "savings")?> >savings</option></select></td>
</tr>
<tr class="AnswerRow0">
<td class="Question_answers ProcessInlineImages">to stage a comeback </td>
<td class="Question_answers" valign="middle"><select onclick="resetAllCombos(this);" size="1" class="TAT_Selects" name="M2"><option value="" <?=$this->IsSelected($bInCorrection, "M2", "")?> ></option><option value="hoard" <?=$this->IsSelected($bInCorrection, "M2", "hoard")?> >hoard</option><option value="re-emerge" <?=$this->IsSelected($bInCorrection, "M2", "re-emerge")?> >re-emerge</option><option value="savings" <?=$this->IsSelected($bInCorrection, "M2", "savings")?> >savings</option></select></td>
</tr>
<tr class="AnswerRow1">
<td class="Question_answers ProcessInlineImages">to squirrel away </td>
<td class="Question_answers" valign="middle"><select onclick="resetAllCombos(this);" size="1" class="TAT_Selects" name="M3"><option value="" <?=$this->IsSelected($bInCorrection, "M3", "")?> ></option><option value="hoard" <?=$this->IsSelected($bInCorrection, "M3", "hoard")?> >hoard</option><option value="re-emerge" <?=$this->IsSelected($bInCorrection, "M3", "re-emerge")?> >re-emerge</option><option value="savings" <?=$this->IsSelected($bInCorrection, "M3", "savings")?> >savings</option></select></td>
</tr>
</table></p>
...
<td valign="top"><img src="_images/t.gif" width="1" height="1"></td>
  ...
  bDontScrollDraggables = true;
	...
	myQuizz.RightAnswer[0] = "savings";

	myQuizz.RightAnswer[1] = "re-emerge";

	myQuizz.RightAnswer[2] = "hoard";
