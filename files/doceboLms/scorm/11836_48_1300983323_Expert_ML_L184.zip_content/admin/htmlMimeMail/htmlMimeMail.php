...
* content-id's.
*/
	function setHtml($html, $text = null,
	...
	function _findHtmlImages($images_dir, $TempImagesDir,
									 				 $RootPath, $RootURLs)
	{
		$extensions = array();
		$html_images = array();
...
*/
	function CompareEmbeddedImages($a, $b)
	{
...
*/
	function _encodeHeader($input, $charset = 'ISO-8859-1', $bUseAlternativeEncoding = false)
	{
		...
		switch ($type) {
			...
			case 'smtp':
				...
				foreach ($this->headers as $name => $value) {
					...
					if ($name == 'Subject')
						$headers[] = $name . ': ' . $this->_encodeHeader($value, $this->build_params['head_charset'], true);
					else
						$headers[] = $name . ': ' . $this->_encodeHeader($value, $this->build_params['head_charset']);
				}
