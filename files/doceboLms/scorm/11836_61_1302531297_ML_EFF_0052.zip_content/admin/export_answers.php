<?php
			...
			if (isset($Question["Answers"]) && is_array($Question["Answers"]))
			{
				foreach ($Question["Answers"] as $ColumnIndex => $QuestionRow)
				{
					switch ($Question["Type"])
					{
						case "SURVEYQCU" :
						case "QCU"    :
							if (isset($ThisAnswer[$QuestionIndex]) &&
						...
						case "SURVEYQCM" :
						case "QCM"    :
							if (isset($ThisAnswer[$QuestionIndex]['checked']) &&
						...
						case "TABQCU" :
							...
							{
								if (isset($ThisAnswer[$QuestionIndex][$ColumnIndex]) &&
						...
						case "SORT"  :
						case "FORM"  :
							if (!isset($ThisAnswer[$QuestionIndex][$ColumnIndex + 1]))
								$ThisAnswer[$QuestionIndex][$ColumnIndex + 1] = '';
	...
	function GetQuestionMoreFieldsCount(&$QuestionAnswers, &$QuestionDefinition, $QuestionIndex)
	{
		...
		foreach ($QuestionAnswers as $answer)
		{
			...
			if (isset($ThisAnswer[$QuestionIndex]) &&
					is_array($ThisAnswer[$QuestionIndex]) &&
					isset($ThisAnswer[$QuestionIndex]['more']))
			{
				if (is_array($ThisAnswer[$QuestionIndex]['more']))
				{
					foreach ($ThisAnswer[$QuestionIndex]['more'] as $aText)
